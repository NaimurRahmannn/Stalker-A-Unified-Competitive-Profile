...
import { getAtCoderAnalytics, getCodeforcesAnalytics, getCompetitiveProgrammingOverview, getLeetCodeAnalytics } from "../api";
...
import { LeetCodeAnalyticsView } from "./leetcode-analytics-view";

type Tab = "overview" | "codeforces" | "atcoder" | "leetcode";
...
function PageHeader({ tab, onTabChange }: { tab: Tab; onTabChange: (tab: Tab) => void }) {
  const tabs: Array<{ id: Tab; label: string }> = [{ id: "overview", label: "Overview" }, { id: "codeforces", label: "Codeforces" }, { id: "atcoder", label: "AtCoder" }, { id: "leetcode", label: "LeetCode" }];
  return <header><div className="flex items-center gap-3"><h1 className="text-[25px] font-semibold leading-tight text-slate-950 sm:text-[28px]">Competitive Programming</h1><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-600 ring-1 ring-emerald-100"><Code2 className="size-5" /></span></div><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 sm:text-base">Track real Codeforces, AtCoder, and LeetCode progress from data cached by STALKER.</p><div role="tablist" aria-label="Competitive programming platform" className="mt-5 inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">{tabs.map((item) => <button key={item.id} type="button" role="tab" aria-selected={tab === item.id} onClick={() => onTabChange(item.id)} className={`h-9 rounded-lg px-4 text-xs font-semibold transition ${tab === item.id ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}>{item.label}</button>)}</div></header>;
}
...
export function CompetitiveProgrammingPageContent() {
  ...
  const [leetcode, setLeetCode] = useState<LeetCodeAnalyticsResponse | null>(null);
  const [errors, setErrors] = useState<Partial<Record<Tab, string>>>({});
  const [isLoading, setIsLoading] = useState(true);
  const [syncing, setSyncing] = useState<Tab | null>(null);
  const [cooldowns, setCooldowns] = useState<Record<Exclude<Tab, "overview">, number>>({ codeforces: 0, atcoder: 0, leetcode: 0 });

  const loadAnalytics = useCallback(async () => {
    const results = await Promise.allSettled([getCompetitiveProgrammingOverview(), getCodeforcesAnalytics(), getAtCoderAnalytics(), getLeetCodeAnalytics()]);
    ...
    setLeetCode((current) => {
      if (results[3].status === "fulfilled") return results[3].value;
      return current;
    });
    setErrors({
      ...
      ...(results[3].status === "rejected" ? { leetcode: getApiErrorMessage(results[3].reason, "Could not load LeetCode analytics.") } : {}),
    });
    const codeforcesResult = results[1];
    const atcoderResult = results[2];
    const leetcodeResult = results[3];
    ...
    if (leetcodeResult.status === "fulfilled") {
      const seconds = leetcodeResult.value.account?.sync_cooldown_seconds ?? 0;
      setCooldowns((value) => ({ ...value, leetcode: seconds }));
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => setCooldowns((value) => ({ codeforces: Math.max(0, value.codeforces - 1), atcoder: Math.max(0, value.atcoder - 1), leetcode: Math.max(0, value.leetcode - 1) })), 1000);
    return () => window.clearInterval(interval);
  }, [cooldowns]);

  const handleSync = async (platform: Exclude<Tab, "overview">) => {
    const account = platform === "codeforces" ? codeforces?.account : platform === "atcoder" ? atcoder?.account : leetcode?.account;
